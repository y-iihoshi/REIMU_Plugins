...
static const char* szPluginInfo[MAX_INFO] = {
	"REIMU Plug-in For 東方非想天則 Ver1.00 (C) IIHOSHI Yoshinori, 2014",
	"東方非想天則",
	"*.rep",
	"東方非想天則 リプレイファイル(*.rep)"
};
...
{
	return CTh123Replay().IsSupported(pSrc, dwSize);
}
...
int GetFileInfoListEx(const char* pSrc, unsigned int dwSize, FileInfo** lpInfo, const char* szFile)
{
	...
	*lpInfo = (FileInfo *)::LocalAlloc(LPTR, sizeof(FileInfo) * MAX_COLUMN);
	...
	std::unique_ptr<CTh123Replay::Info> info(replay.GetInfo(pSrc, dwSize));
	if (info.get() == NULL)
		return RPI_FILE_READ_ERROR;

	FileInfo *pInfo = (FileInfo *)*lpInfo;
	::sprintf_s(pInfo[CL_VERSION].szText, "%s", Th123::GetString(Th123::Versions, info->versionId).c_str());
	::sprintf_s(pInfo[CL_MODE   ].szText, "%s", Th123::GetString(Th123::Modes, info->mode).c_str());
	::sprintf_s(pInfo[CL_PLAYER1].szText, "%s",
		Th123::GetString(Th123::Characters, info->pDeck1->chara).c_str());
	::sprintf_s(pInfo[CL_COLOR1 ].szText, "%hhd", info->pDeck1->color);
	if (info->mode != Th123::Mode::Story)
	{
		::sprintf_s(pInfo[CL_PLAYER2].szText, "%s",
			Th123::GetString(Th123::Characters, info->pDeck2->chara).c_str());
		::sprintf_s(pInfo[CL_COLOR2 ].szText, "%hhd", info->pDeck2->color);
		::sprintf_s(pInfo[CL_STAGE  ].szText, "%s", Th123::GetString(Th123::Stages, info->stageId).c_str());
		::sprintf_s(pInfo[CL_BGM    ].szText, "%s", Th123::GetString(Th123::Bgms, info->bgmId).c_str());
	}
	else
	{
