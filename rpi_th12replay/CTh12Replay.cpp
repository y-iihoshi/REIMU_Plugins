#include <regex>
#include <cstring>
#include "CTh12Replay.h"
#include "..\rpi_threplay\smart_array.h"
#include "..\rpi_threplay\sueLib_LE.h"

CTh12Replay::Info::Info(const char *pSrc, size_t nSize)
	: pData(new char[nSize])
{
	...
	this->pszRank = std::strstr(this->pData, "Rank ") + std::strlen("Rank ");
...
CTh12Replay::Info::~Info()
{
	delete [] this->pData;
}

/*
	Structure of TH12 replay file:
		replay
			0x24 bytes:	replayHeader
				0x04 bytes:	replaySignature ("t12r")
				0x08 bytes:	(unknown)
...
bool CTh12Replay::IsSupported(const char *pSrc, size_t nSize)
{
	return (nSize >= 4) && (std::strncmp(pSrc, "t12r", 4) == 0);
}

int CTh12Replay::GetInfo(const char *pSrc, size_t nSrcSize, const char **ppDst, size_t *pDstSize)
{
	return this->GetUserData(USER_DATA_TYPE_INFO, pSrc, nSrcSize, ppDst, pDstSize);
}

CTh12Replay::Info *CTh12Replay::GetInfo(const char *pSrc, size_t nSrcSize)
{
	const char *pDest = NULL;
	size_t nDestSize = 0;

	return (this->GetInfo(pSrc, nSrcSize, &pDest, &nDestSize) == RET_OK)
		? new CTh12Replay::Info(pDest, nDestSize) : NULL;
}

int CTh12Replay::GetComment(const char *pSrc, size_t nSrcSize, const char **ppDst, size_t *pDstSize)
{
	return this->GetUserData(USER_DATA_TYPE_COMMENT, pSrc, nSrcSize, ppDst, pDstSize);
}

int CTh12Replay::ReadComment(const char *pFilename, const char **ppDst, size_t *pDstSize)
{
...
int CTh12Replay::WriteComment(const char *pFilename, const char *pSrc, size_t nSrcSize)
{
	smart_array<char> buf;
...
int CTh12Replay::GetNumber(const char *pFilename, int *pNumType, char *pNum, size_t nNumSize)
{
	...
	std::regex reBasic("th12_([[:digit:]]{2}).rpy", (std::regex::ECMAScript | std::regex::icase));
	std::regex reUser("th12_ud(.{0,4}).rpy", (std::regex::ECMAScript | std::regex::icase));
	std::cmatch results;
...
int CTh12Replay::GetUserData(int nType, const char *pSrc, size_t nSize, const char **ppDst, size_t *pDstSize)
{
