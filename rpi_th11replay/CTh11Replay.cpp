...
CTh11Replay::Info::~Info()
{
	delete [] this->pData;
}

/*
	Structure of TH11 replay file:
		replay
			0x24 bytes:	replayHeader
				0x04 bytes:	replaySignature ("t11r")
				0x08 bytes:	(unknown)
...
bool CTh11Replay::IsSupported(const char *pSrc, size_t nSize)
{
	return (nSize >= 4) && (std::strncmp(pSrc, "t11r", 4) == 0);
}

int CTh11Replay::GetInfo(const char *pSrc, size_t nSrcSize, const char **ppDst, size_t *pDstSize)
{
	return this->GetUserData(USER_DATA_TYPE_INFO, pSrc, nSrcSize, ppDst, pDstSize);
}

CTh11Replay::Info *CTh11Replay::GetInfo(const char *pSrc, size_t nSrcSize)
{
	const char *pDest;
	size_t nDestSize;

	return (this->GetInfo(pSrc, nSrcSize, &pDest, &nDestSize) == RET_OK)
		? new CTh11Replay::Info(pDest, nDestSize) : NULL;
}

int CTh11Replay::GetComment(const char *pSrc, size_t nSrcSize, const char **ppDst, size_t *pDstSize)
{
	return this->GetUserData(USER_DATA_TYPE_COMMENT, pSrc, nSrcSize, ppDst, pDstSize);
}

int CTh11Replay::ReadComment(const char *pFilename, const char **ppDst, size_t *pDstSize)
{
...
int CTh11Replay::WriteComment(const char *pFilename, const char *pSrc, size_t nSrcSize)
{
	smart_array<char> buf;
...
int CTh11Replay::GetNumber(const char *pFilename, int *pNumType, char *pNum, size_t nNumSize)
{
	...
	std::regex reBasic("th11_([[:digit:]]{2}).rpy", (std::regex::ECMAScript | std::regex::icase));
	std::regex reUser("th11_ud(.{0,4}).rpy", (std::regex::ECMAScript | std::regex::icase));
	std::cmatch results;
...
int CTh11Replay::GetUserData(int nType, const char *pSrc, size_t nSize, const char **ppDst, size_t *pDstSize)
{
