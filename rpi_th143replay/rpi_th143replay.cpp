...
static const char* szPluginInfo[MAX_INFO] = {
	"REIMU Plug-in For 弾幕アマノジャク Ver1.00 (C) IIHOSHI Yoshinori, 2014",
	"弾幕アマノジャク",
	"th143_*.rpy",
	"弾幕アマノジャク リプレイファイル(th143_*.rpy)"
};
...
{
	return CTh143Replay().IsSupported(pSrc, dwSize);
}
...
int GetFileInfoListEx(const char* pSrc, unsigned int dwSize, FileInfo** lpInfo, const char* szFile)
{
	...
	*lpInfo = (FileInfo *)::LocalAlloc(LPTR, sizeof(FileInfo) * MAX_COLUMN);
	...
	std::unique_ptr<CTh143Replay::Info> info(replay.GetInfo(pSrc, dwSize));
	...
	case RET_OK:
		{
			...
			if (replay.GetNumber(szFile, &numType, number, sizeof(number)) == RET_OK)
				switch (numType)
				{
				case CTh143Replay::NUM_TYPE_BASIC:
					::sprintf_s(pInfo[CL_NUMBER].szText, "No.%2s", number);
					break;
				case CTh143Replay::NUM_TYPE_USER:
			...
			::sprintf_s(pInfo[CL_DAY].szText, "%s", info->pszDay);
			::sprintf_s(pInfo[CL_SCENE].szText, "%s", info->pszScene);
