...
CTh105Replay::Deck::Deck(const char *pSrc)
{
	if (pSrc != nullptr)
	{
...
CTh105Replay::Deck::~Deck()
{
}

CTh105Replay::Info::Info(const char *pSrc, size_t nSize)
...
CTh105Replay::Info::~Info()
{
	delete pDeck1;
	delete pDeck2;
}

bool CTh105Replay::IsSupported(const char *pSrc, size_t nSize)
{
	...
	if (nSize >= 2)
	{
		...
		isSupported = (Th105::Versions.find(versionId) != Th105::Versions.cend());
	}

	return isSupported;
}

int CTh105Replay::GetPlayerInfo(
	int player, const char *pSrc, size_t nSrcSize, char **ppDst, size_t *pDstSize)
{
	...
	CTh105Replay::Info info(pSrc, nSrcSize);
	...
	*ppDst = (char *)::LocalAlloc(LPTR, 2048);
	...
	::print(&pDst, &nRemainSize, "Character: %s\r\n",
		Th105::GetString(Th105::Characters, pDeck->chara).c_str());
	::print(&pDst, &nRemainSize, "Color: %d\r\n", pDeck->color);
	std::for_each(pDeck->cards.cbegin(), pDeck->cards.cend(),
		[&](const Deck::CardMap::value_type &value)
		{
			const auto &cards = (value.first < 100) ? Th105::SystemCards : Th105::Cards.at(pDeck->chara);
			return ::print(&pDst, &nRemainSize, "%s * %d\r\n",
				Th105::GetString(cards, value.first).c_str(), value.second);
		});
...
CTh105Replay::Info *CTh105Replay::GetInfo(const char *pSrc, size_t nSrcSize)
{
	return new CTh105Replay::Info(pSrc, nSrcSize);
}
