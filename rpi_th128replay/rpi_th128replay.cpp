...
static const ColumnInfo Column[MAX_COLUMN + 1] = {
	...
	{"ルート",			LVCFMT_LEFT,	SORT_STRING,	CLSYS_STRING},
...
static const char* szPluginInfo[MAX_INFO] = {
	"REIMU Plug-in For 妖精大戦争 Ver1.01 (C) IIHOSHI Yoshinori, 2013-2014",
	"妖精大戦争",
	"th128_*.rpy",
	"妖精大戦争 リプレイファイル(th128_*.rpy)"
};
...
{
	return CTh128Replay().IsSupported(pSrc, dwSize);
}
...
int GetFileInfoListEx(const char* pSrc, unsigned int dwSize, FileInfo** lpInfo, const char* szFile)
{
	...
	*lpInfo = (FileInfo *)::LocalAlloc(LPTR, sizeof(FileInfo) * MAX_COLUMN);
	...
	std::unique_ptr<CTh128Replay::Info> info(replay.GetInfo(pSrc, dwSize));
	...
	case RET_OK:
		{
			...
			if (replay.GetNumber(szFile, &numType, number, sizeof(number)) == RET_OK)
				switch (numType)
				{
				case CTh128Replay::NUM_TYPE_BASIC:
					::sprintf_s(pInfo[CL_NUMBER].szText, "No.%2s", number);
					break;
				case CTh128Replay::NUM_TYPE_USER:
			...
			::sprintf_s(pInfo[CL_ROUTE].szText, "%s", info->pszRoute);
			::sprintf_s(pInfo[CL_LEVEL].szText, "%s", info->pszLevel);
