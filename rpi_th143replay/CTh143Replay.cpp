...
CTh143Replay::Info::Info(const char *pSrc, size_t nSize)
	: pData(new char[nSize])
{
	...
	this->pszDay = std::strstr(this->pData, "Day ") + std::strlen("Day ");
	this->pszScene = std::strstr(this->pData, "Scene ") + std::strlen("Scene ");
...
CTh143Replay::Info::~Info()
{
	delete [] this->pData;
}

/*
	Structure of TH143 replay file:
		replay
			0x24 bytes:	replayHeader
				0x04 bytes:	replaySignature ("t143")
				0x08 bytes:	(unknown)
...
bool CTh143Replay::IsSupported(const char *pSrc, size_t nSize)
{
	return (nSize >= 4) && (std::strncmp(pSrc, "t143", 4) == 0);
}

int CTh143Replay::GetInfo(const char *pSrc, size_t nSrcSize, const char **ppDst, size_t *pDstSize)
{
	return this->GetUserData(USER_DATA_TYPE_INFO, pSrc, nSrcSize, ppDst, pDstSize);
}

CTh143Replay::Info *CTh143Replay::GetInfo(const char *pSrc, size_t nSrcSize)
{
	const char *pDest = NULL;
	size_t nDestSize = 0;

	return (this->GetInfo(pSrc, nSrcSize, &pDest, &nDestSize) == RET_OK)
		? new CTh143Replay::Info(pDest, nDestSize) : NULL;
}

int CTh143Replay::GetComment(const char *pSrc, size_t nSrcSize, const char **ppDst, size_t *pDstSize)
{
	return this->GetUserData(USER_DATA_TYPE_COMMENT, pSrc, nSrcSize, ppDst, pDstSize);
}

int CTh143Replay::ReadComment(const char *pFilename, const char **ppDst, size_t *pDstSize)
{
...
int CTh143Replay::WriteComment(const char *pFilename, const char *pSrc, size_t nSrcSize)
{
	smart_array<char> buf;
...
int CTh143Replay::GetNumber(const char *pFilename, int *pNumType, char *pNum, size_t nNumSize)
{
	...
	std::regex reBasic("th143_([[:digit:]]{2}).rpy", (std::regex::ECMAScript | std::regex::icase));
	std::regex reUser("th143_ud(.{0,4}).rpy", (std::regex::ECMAScript | std::regex::icase));
	std::cmatch results;
...
int CTh143Replay::GetUserData(int nType, const char *pSrc, size_t nSize, const char **ppDst, size_t *pDstSize)
{
