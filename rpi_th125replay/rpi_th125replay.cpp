...
static const char* szPluginInfo[MAX_INFO] = {
	"REIMU Plug-in For ダブルスポイラー Ver1.02 (C) IIHOSHI Yoshinori, 2013-2014",
	"ダブルスポイラー",
	"th125_*.rpy",
...
{
	return CTh125Replay().IsSupported(pSrc, dwSize);
}
...
int GetFileInfoListEx(const char* pSrc, unsigned int dwSize, FileInfo** lpInfo, const char* szFile)
{
	...
	*lpInfo = (FileInfo *)::LocalAlloc(LPTR, sizeof(FileInfo) * MAX_COLUMN);
	...
	std::unique_ptr<CTh125Replay::Info> info(replay.GetInfo(pSrc, dwSize));
	...
	case RET_OK:
		{
			...
			if (replay.GetNumber(szFile, &numType, number, sizeof(number)) == RET_OK)
				switch (numType)
				{
				case CTh125Replay::NUM_TYPE_BASIC:
					::sprintf_s(pInfo[CL_NUMBER].szText, "No.%2s", number);
					break;
				case CTh125Replay::NUM_TYPE_USER:
			...
			::sprintf_s(pInfo[CL_LEVELSCENE].szText, "%s", info->pszLevelScene);
