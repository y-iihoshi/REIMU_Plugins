#include <regex>
#include <cstring>
#include "CTh125Replay.h"
#include "..\rpi_threplay\smart_array.h"
#include "..\rpi_threplay\sueLib_LE.h"

CTh125Replay::Info::Info(const char *pSrc, size_t nSize)
	: pData(new char[nSize])
{
	...
	this->pszLevelScene = std::strstr(this->pData, "Score ") - 6;	// "??-?\r\n"
...
CTh125Replay::Info::~Info()
{
	delete [] this->pData;
}

/*
	Structure of TH125 replay file:
		replay
			0x24 bytes:	replayHeader
				0x04 bytes:	replaySignature ("t125")
				0x08 bytes:	(unknown)
...
bool CTh125Replay::IsSupported(const char *pSrc, size_t nSize)
{
	return (nSize >= 4) && (std::strncmp(pSrc, "t125", 4) == 0);
}

int CTh125Replay::GetInfo(const char *pSrc, size_t nSrcSize, const char **ppDst, size_t *pDstSize)
{
	return this->GetUserData(USER_DATA_TYPE_INFO, pSrc, nSrcSize, ppDst, pDstSize);
}

CTh125Replay::Info *CTh125Replay::GetInfo(const char *pSrc, size_t nSrcSize)
{
	const char *pDest = NULL;
	size_t nDestSize = 0;

	return (this->GetInfo(pSrc, nSrcSize, &pDest, &nDestSize) == RET_OK)
		? new CTh125Replay::Info(pDest, nDestSize) : NULL;
}

int CTh125Replay::GetComment(const char *pSrc, size_t nSrcSize, const char **ppDst, size_t *pDstSize)
{
	return this->GetUserData(USER_DATA_TYPE_COMMENT, pSrc, nSrcSize, ppDst, pDstSize);
}

int CTh125Replay::ReadComment(const char *pFilename, const char **ppDst, size_t *pDstSize)
{
...
int CTh125Replay::WriteComment(const char *pFilename, const char *pSrc, size_t nSrcSize)
{
	smart_array<char> buf;
...
int CTh125Replay::GetNumber(const char *pFilename, int *pNumType, char *pNum, size_t nNumSize)
{
	...
	std::regex reBasic("th125_([[:digit:]]{2}).rpy", (std::regex::ECMAScript | std::regex::icase));
	std::regex reUser("th125_ud(.{0,4}).rpy", (std::regex::ECMAScript | std::regex::icase));
	std::cmatch results;
...
int CTh125Replay::GetUserData(int nType, const char *pSrc, size_t nSize, const char **ppDst, size_t *pDstSize)
{
